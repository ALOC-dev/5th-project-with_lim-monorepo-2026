...
import { OperationVerifier } from "./operation-hours.js";
...
export const mergeEvidenceWithEnrichment = (
  ...
  placeInfo: {
    ...evidence.placeInfo,
    priceRangePerPerson:
      enrichment.priceRangePerPerson ?? evidence.placeInfo.priceRangePerPerson,
  },
  ...
  trustSignals: {
    ...
    evidenceUrls: Array.from(
      new Set([
        ...evidence.trustSignals.evidenceUrls,
  ...
  accessibilitySignals: {
    ...
    openTimeBufferMinutes:
      enrichment.operationVerification.status === "OPEN" ? 0 : undefined,
  },
  enrichment,
});

// UNKNOWN을 OPEN으로 승격하지 않는다. 실제 operationInfo가 있어야 scoring 대상이 된다.
export const shouldRecommendByOperationHours = (
  enrichment: CandidateEnrichment,
): boolean =>
