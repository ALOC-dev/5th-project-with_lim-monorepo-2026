...
const BLOCKED_SOURCE_PATH_PATTERN = /\.(?:pdf|xls|xlsx|csv|zip|hwp|doc|docx|ppt|pptx)(?:$|[?#])/iu;
...
export const isUsableEvidenceUrl = (url: string): boolean => {
  ...
  if (BLOCKED_SOURCE_HOST_PATTERNS.some((pattern) => pattern.test(parsed.hostname))) {
    return false;
  }
  const pathAndQuery = `${parsed.pathname}${parsed.search}`;
  if (parsed.searchParams.has("download") || BLOCKED_SOURCE_DOWNLOAD_PATTERN.test(pathAndQuery)) {
    return false;
  }
