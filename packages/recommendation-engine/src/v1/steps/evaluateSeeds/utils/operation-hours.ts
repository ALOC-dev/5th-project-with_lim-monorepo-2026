...
const parseCurrentOperationStatus = (
  ...
  // "영업 종료 11:00에 영업 시작"을 요청 요일의 휴무로 단정하면 안 된다.
  //
  // 이 문구는 지도 패널의 **현재 상태**다. "지금은 영업 전이고 11:00에 연다"는
  // 뜻이지 "그날 하루 쉰다"가 아니다. 우리가 검증하는 건 지금이 아니라 사용자가
  // 요청한 날짜·시각이므로, 11시에 여는 가게를 토요일 18시 요청에서 CLOSED로
  // 판정하는 건 정반대다.
  //
  // 실제로 카카오에서 연중무휴 09:00~03:00으로 확인된 가게가 네이버의 이 한 줄
  // 때문에 "일요일 휴무"로 탈락했다. 없는 정보를 만들어내는 오탐이라 제거한다.
  // 여는 시각만 있고 닫는 시각이 없으므로 스케줄을 만들 수 없다고 보는 게 맞다.
  return [];
};
