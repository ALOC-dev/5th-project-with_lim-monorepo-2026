  ...
  const variants = names.flatMap((nameAlias, index) => {
    ...
    return [
      ...
      toVariant(
        [nameAlias],
        nameAlias,
        isOriginal ? "name_only" : "alias_only",
      ),
    ];
  });
...
export const buildReferenceNameAliases = (name: string): string[] => {
...
export const scoreTextReferenceIdentity = (
  ...
  const containsName = normalizedText.includes(
    normalizeComparableText(expected.name),
  );
...
const stripBranchSuffix = (name: string): string =>
  name.replace(
    /\s+[가-힣A-Za-z0-9]+(?:역사점|신분당선점|지하상가점|아라타워점|역점|점)$/u,
    "",
  );
...
const toReferenceIdentityScore = ({
  ...
  const distanceScore =
    distanceMeters === undefined
      ? undefined
      : clamp01(1 - Math.min(distanceMeters, MAX_DISTANCE_SCORE_METERS) / MAX_DISTANCE_SCORE_METERS);
  const weightedIdentity =
    nameScore * 0.55 + addressScore * 0.3 + (distanceScore ?? 0.4) * 0.15;
  ...
  if (
    ...
    return {
      ...
      identityScore: Math.max(
        weightedIdentity,
        nameScore * 0.85 + (distanceScore ?? 0) * 0.15,
      ),
      accepted: true,
