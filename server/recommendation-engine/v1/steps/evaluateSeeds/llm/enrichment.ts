...
// 모델에는 "어떤 벤더를 어떤 순서로 쓰면 좋은지"만 알려준다.
// 실제 API/scraper 호출과 parser는 tools/vendors와 utils에 남겨 LLM 출력에 의존하지 않는다.
...
export const createAgenticWebEnrichmentClient = ({
  ...
  clientId = process.env.NAVER_SEARCH_CLIENT_ID ?? process.env.NAVER_CLIENT_ID,
  clientSecret =
  ...
  return async ({ userInput, evidences }) => {
    ...
    // Naver Search key가 없으면 agentic web 보강은 의미 있게 진행할 수 없다.
    // 이 경우도 후보별 UNKNOWN enrichment로 돌려보내 downstream gate가 처리하게 한다.
    ...
    // request 전체에 단 하나의 브라우저만 launch. 모든 후보·모든 browser tool이 lazy하게 공유한다.
    // 후보별 병렬 실행은 유지하되 Playwright process 폭증은 막기 위한 장치다.
    ...
    try {
      return await mapWithConcurrency(
        ...
        maxConcurrency,
        async (evidence) => {
          // 후보 하나가 오래 매달려도 전체 evaluate stage를 붙잡지 않도록 candidate 단위 timeout을 둔다.
          ...
          try {
            ...
            // 개별 후보 enrichment 실패는 stage 실패로 올리지 않는다.
            // 다른 후보가 추천 가능하면 계속 진행하고, 실패 후보만 UNKNOWN으로 gate에서 빠진다.
...
const mapWithConcurrency = async <TItem, TResult>(
  ...
  const workers = Array.from(
    { length: normalizeConcurrency(concurrency, items.length) },
    async () => {
...
> & {
  ...
  // 같은 후보 안에서 여러 tool이 동일 URL을 두 번 fetch하지 않도록 공유한다.
  scrapeRequests: Map<string, Promise<UrlScrapeResult>>;
  // request-scoped 브라우저. 처음 호출되는 lookup이 lazy launch.
  getBrowser: () => Promise<PlaywrightBrowser>;
  onToolEvent?: (event: AgenticWebEnrichmentToolEvent) => void;
  // tool별 startTimer + 실패 로깅용. extra context로 candidateId가 자동 누적된다.
  logger?: Logger;
...
const enrichWithAgenticWeb = async (
  ...
  // createAgenticEnrichmentTools는 LLM tool 호출 결과를 memo/finalized에 기록한다.
  // generateText의 text 자체는 버리고, final tool call의 구조화된 결과만 신뢰한다.
  ...
  if (!finalized) {
    // step limit 때문에 finalize를 못 했더라도 이미 deterministic tool이 OPEN/CLOSED를 찾았다면
    // 그 근거를 버리지 않는다. memo에도 판단 가능한 근거가 없을 때만 UNKNOWN으로 둔다.
    return (
...
const buildFinalEnrichmentFromAgentic = async (
  ...
  if (finalized.selectedSource === "agentic") {
    // agentic source는 모델이 fetch/search로 모은 raw snippet을 직접 해석하는 경로다.
    // parser가 실제 영업시간을 뽑지 못하면 OPEN으로 만들지 않고 UNKNOWN으로 남긴다.
  ...
  // 외부 source 채택: tool memo의 결정적 결과를 base로 두고, LLM이 추가로 채운 신호만 trustSignals에 머지.
  // 단, 모델이 UNKNOWN source를 골랐는데 다른 memo에 OPEN/CLOSED 근거가 있으면 그 결정적 근거를 우선한다.
