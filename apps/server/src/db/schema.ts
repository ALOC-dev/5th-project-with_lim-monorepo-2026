...
export const courses = pgTable(
  "courses",
  {
    ...
    candidateDecisions: jsonb("candidate_decisions")
      .$type<readonly CourseCandidateDecision[]>()
      .notNull()
      .default(sql`'[]'`),
...
export const courseOptions = pgTable(
  "course_options",
  {
    ...
    routePath: jsonb("route_path")
      .$type<readonly CourseRoutePoint[]>()
      .notNull()
      .default(sql`'[]'`),
...
export const coursePlaces = pgTable(
  "course_places",
  {
    ...
    favoritePlaceId: uuid("favorite_place_id").references(() => favoritePlaces.id, {
      onDelete: "set null",
    }), // source='FAVORITE'일 때만
  ...
  (table) => [
    check(
      "chk_course_place_source",
      sql`${table.source} IN ('FAVORITE', 'KAKAO', 'SAVED_PLACE', 'DIRECT_SEARCH', 'RECOMMENDATION')`,
    ),
...
export const savedCourseOptions = pgTable(
  "saved_course_options",
  {
    ...
    userId: uuid("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    sourceCourseOptionId: uuid("source_course_option_id").references(() => courseOptions.id, {
      onDelete: "set null",
    }),
